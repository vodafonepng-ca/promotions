...
if(isset($_POST['uid'])&&isset($_POST['super_admin'])&&isset($_POST['portal_admin'])&&isset($_POST['content_creator'])&&isset($_POST['portal_user'])&&isset($_POST['fullname'])&&isset($_POST['phone'])&&isset($_POST['email'])&&isset($_POST['username'])
&&isset($_POST['upassword'])&&isset($_POST["status"])){
      ...
      // $phonesql="SELECT* FROM users WHERE phone='$phone'";
        ...
        try {
          ...
          if($password!==""){
              $hasseded=md5($password);
            $sql="UPDATE users SET fullname=:fullname,username=:username,email=:email,phone=:phone,password=:password,create_promo_prev=:create_promo_prev,update_promo_prev=:update_promo_prev,create_user_prev=:create_user_prev,update_user_prev=:update_user_prev,delete_user_prev=:delete_user_prev,accout_status=:accout_status WHERE id ='$_POST[uid]'";
            ...
            $sql="UPDATE users SET fullname=:fullname,username=:username,email=:email,phone=:phone,create_promo_prev=:create_promo_prev,update_promo_prev=:update_promo_prev,create_user_prev=:create_user_prev,update_user_prev=:update_user_prev,delete_user_prev=:delete_user_prev,accout_status=:accout_status WHERE id ='$_POST[uid]'";
            $stmt=$conn->prepare($sql)->execute([
              ':fullname'=>$fullname,
              ':username'=>$username,
              ':email'=>$email,
          ...
          if($stmt){
            ...
            $response['message']="User inoformation Updated";
            echo json_encode($response);
            die();
          }else {
            $response['response']="error";
