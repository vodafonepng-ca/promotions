              ...
              success:(res)=>{
                  ...
                  if(data.status==="error"){
                      ...
                      promodata=data.message
                       ...
                       promodata.forEach((element)=>{
                        $(dataTable).append(`<tr id='${element.id}'><td id=''>${element.id}</td><td>${element.fullname}</td><td>${element.phone}</td><td>${element.email}</td><td class='${element.accout_status=='Active'?'text-success':''}'>${element.accout_status}</td><td><a  href="#" class='btn btn-primary btn-sm ' data-target='#modal-lg'  data-toggle='modal' title='View user information' onClick='viewUser(${element.id})'> <i class="bi bi-eye"></i></a> <a href="#" class='btn btn-primary btn-sm update-promo' data-target='#modal-lg' title='Update user information' data-toggle='modal' onClick='updateUser(${element.id})'> <i class="bi bi-pencil-square"></i>Update</a> ${element.create_user_prev==`true`?``:`<a href="#" class='btn btn-danger btn-sm del-promo' onClick='deleteUser(${element.id})' data-toggle="modal" data-target="#modal-sm-center" title='Delete this user' disabled> <i class="bi bi-trash" ></i> </a>`}</td></tr>`)
                       })
                      $("#admin-pannel").html(dataTable)
                      contentLoaded='users'
                  }
