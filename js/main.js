...
$("#shw-results").on("click", () => {
    ...
    if(rsults.length===0){
        ...
        $("#modal-1-body").html(`<div>${rsults.length} Lucky Winners <button class='btn btn-primary float-right ' title='Download Results' id='dlbtn' onClick='DownloadResults()'><i class='bi bi-box-arrow-down'></i> Download Results</buttton> </div><hr>`)
...
function reset() {
    $.ajax({
        type:"GET",
        url:"/promotions/draws/reset-session",
        beforeSend:()=>{
